...
main ()
{
    ...
    if runningLinux; then
        cyan 'You are running linux!  Downloading findref v1.2.2 for linux...'
        bin_name='findref'
        downlink_link "${dest_dir}" "$(linux_link)"
    elif runningOSX; then
        cyan 'You are running macOS!  Downloading findref v1.2.2 for macOS...'
        bin_name='findref'
    ...
    cyan 'Unzipping v1.2.2'
    unzip 'findref.zip' || die 'Error unzipping findref.zip'

    cyan 'Cleaning up the zip file for v1.2.2'
    rm -f 'findref.zip' || red "Error removing findref.zip"

    cyan "Making ${bin_name} v1.2.2 executable"
